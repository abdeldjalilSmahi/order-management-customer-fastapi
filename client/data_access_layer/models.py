...
class CustomerDalModel(Base):
    ...
    customer_number: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    customer_id: Mapped[int] = mapped_column(unique=True, nullable=True)
    ...
    def __repr__(self):
        data = tuple((self.customer_number, self.customer_id, self.firstname, self.lastname, self.email, self.phone_number))
        return data.__str__()
...
class OrderDalModel(Base):
    ...
    customer_number: Mapped[int] = mapped_column(ForeignKey("Customers.customer_number", ondelete="CASCADE"))
    ...
    def __repr__(self):
        data = tuple((self.order_id, self.order_date, self.customer_number, self.actual_status.name))
        return data.__str__()
